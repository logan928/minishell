...
/*   Updated: 2025/08/06 13:47:54 by mkugan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_token_kind	ft_get_operator_kind(const char *s)
{
	if (s[0] == '|' && s[1] == '|')
		return (OR_IF);
	else if (s[0] == '|')
...
void	lex(t_lexer *lexer)
{
	...
	while (lexer->src[lexer->pos])
	{
		...
		else if (ft_isquote(lexer->src[lexer->pos]))
		{
			...
			while (lexer->src[lexer->pos] && !ft_isquote(lexer->src[lexer->pos]))
				lexer->pos++;
		...
		{
			kind = ft_get_operator_kind(&lexer->src[lexer->pos]);
