...
void	ft_append_unquoted_quote(t_shell *s, t_token *t, char **res)
{
	...
	s->lexer->quote = t->data[s->lexer->pos];
	quote = ft_strndup_safe(s, &t->data[s->lexer->pos], 1);
...
void	ft_append_variable(t_shell *s, t_token *t, char **res)
{
	...
	while (t->data[s->lexer->pos] && ft_valid_env_char(t->data[s->lexer->pos]))
		s->lexer->pos++;
	start = s->lexer->start;
	len = s->lexer->pos - start;
	var = ft_get_env_var(s, &t->data[start], len, s->env);
...
void	ft_append_normal_chars(t_shell *s, t_token *t, char **res)
{
	...
	s->lexer->start = s->lexer->pos++;
	while (t->data[s->lexer->pos]
		&& !ft_isquote(t->data[s->lexer->pos])
		&& t->data[s->lexer->pos] != '$')
		s->lexer->pos++;
	start = s->lexer->start;
	len = s->lexer->pos - start;
	str = ft_strndup_safe(s, &t->data[s->lexer->start], len);
