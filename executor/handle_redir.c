...
static int	handle_r_in(t_shell *shell, t_redir *redir, int shell_type, \
	t_command_kind kind)
{
	int	fd;

	fd = open_file(shell, redir, shell_type, O_RDONLY);
	if (fd < 0)
		return (1);
	if (kind != BUILTIN)
	{
...
static int	handle_r_out(t_shell *shell, t_redir *redir, int shell_type)
{
	int	fd;
...
static int	handle_r_app(t_shell *shell, t_redir *redir, int shell_type)
{
	int	fd;
...
{
	if (redir->kind == R_IN)
		return (handle_r_in(shell, redir, shell_type, kind));
	else if (redir->kind == R_OUT)
		return (handle_r_out(shell, redir, shell_type));
	else if (redir->kind == R_APP)
		return (handle_r_app(shell, redir, shell_type));
